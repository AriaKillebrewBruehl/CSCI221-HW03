// Aria Killebrew Breuhl, HW03 Part 1 (with help from the awesome drop-in tutors!!)
// llist.cc
//
...
namespace llist {
  ...
  //
  int length(llist* list) {
    int count = 0; // initialize a variable to count the length of the list 
    node* current = list->first;
  ...
  void remove(llist* list, int value) {
    node* prev = nullptr; // create a follwer node that starts as nullptr
    node* current = list->first; //create a leader node that starts at the start of the list 

    if (current->data == value) { // if the `value` is the data of the first node just deleteFront(list)
      deleteFront(list);
      return;
    }
    while(current != nullptr && current->data != value) { // traverse the list until the end or the `value` is found
      prev = current;
      current = current->next;
    } 
    prev->next = current->next; // link the follower to the leader's next
    delete current; // delete the `value` that was to be removed 
    return;  
  }
